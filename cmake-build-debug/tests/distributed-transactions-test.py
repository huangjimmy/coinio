...
###############################################################
# Test for transfering currency across multiple accounts in a CPS cluster.
...
def killall():
    if 0 != subprocess.call(["programs/launcher/launcher", "-k", "15"]):
        print("ERROR: Launcher failed to shut down cps cluster.")
...
def waitForCluserStability (ports=[], timeout=60):
    ...
    while time.time()-startTime < timeout:
        ...
        for port in ports:
            ...
            if 0 != subprocess.call(["programs/cpsc/cpsc", "--port", str(port), "get", "block", "1"],
...
def createAccountInfos(count):
    ...
    for i in range(0, count):
        try:
            keyStr=subprocess.check_output(["programs/cpsc/cpsc", "create", "key"]).decode("utf-8")
            m=p.match(keyStr)
            if m is None:
...
def createWallet(INITA_PRV_KEY, accountInfos):
    ...
    if 0 != subprocess.call(["programs/cpsc/cpsc", "wallet", "create", "--name", WALLET_NAME], stdout=FNULL):
        print("ERROR: Failed to create wallet.")
        return False

    if 0 != subprocess.call(["programs/cpsc/cpsc", "wallet", "import", "--name", WALLET_NAME, INITA_PRV_KEY],
                            stdout=FNULL):
    ...
    for accountInfo in accountInfos:
        ...
        if 0 != subprocess.call(["programs/cpsc/cpsc", "wallet", "import", "--name", WALLET_NAME,
...
def getHeadBlockNum(port):
    num=-1
    try:
        ret=subprocess.check_output(["programs/cpsc/cpsc", "--port", str(port), "get", "info"]).decode("utf-8")
        #print("cpsc get info: ", ret)
...
def transferFunds(source, destination, amount, port=START_PORT):
    if 0 != subprocess.call(["programs/cpsc/cpsc", "--port", str(port), "transfer", source, destination,
...
def spreadFunds(source, accountInfos, waitNextBlock=True):
    ...
    print("Transfering", transferAmount, "units from account "+ fromm+ " to "+ to+ " on cps server port ", port)
    if False == transferFunds(fromm, to, transferAmount, port):
        return False

    for i in range(0, count):
        if waitNextBlock:
            print("Waiting on next block on cps server port", port)
        ...
        print("Transfering", transferAmount, "units from account "+ fromm+ " to "+ to+ " on cps server port ",
              port)
...
def getAccountBalance(name):
    try:
        ret=subprocess.check_output(["programs/cpsc/cpsc", "get", "account", name]).decode("utf-8")
        #print ("getAccountBalance>", ret)
        p=re.compile('\n\s+\"cps_balance\"\:\s+\"(\d+.\d+)\s+CPS\",\n', re.MULTILINE)
        m=p.search(ret)
        if m is None:
...
def createAccount(accountInfo):
    ...
    if 0 != subprocess.call(["programs/cpsc/cpsc", "create", "account", ADMIN_ACCOUNT, accountInfo.name, accountInfo.ownerPublic, accountInfo.activePublic], stdout=FNULL):
    ...
    try :
        ret=subprocess.check_output(["programs/cpsc/cpsc", "get", "account", accountInfo.name]).decode("utf-8")
        #print ("ret:", ret)
        p=re.compile('\n\s+\"staked_balance\"\:\s+\"\d+.\d+\s+CPS\",\n', re.MULTILINE)
        m=p.search(ret)
        if m is None:
...
try:
    launcherOpts="--cpsd \"--plugin cpsio::wallet_api_plugin\" -p {0} -n {1} -s {2} -d {3}".format(pnodes, total_nodes, topo, delay)
    print ("launcher options:", launcherOpts)

    if 0 != subprocess.call(["programs/launcher/launcher", "--cpsd", "--plugin cpsio::wallet_api_plugin", "-p", str(pnodes), "-n", str(total_nodes), "-d",
                             str(delay), "-s", topo]):
        errorExit("Launcher failed to stand up cps cluster.")
