...
namespace cpsio { namespace detail {
...
FC_REFLECT(cpsio::detail::faucet_testnet_empty, );
FC_REFLECT(cpsio::detail::faucet_testnet_keys, (owner)(active));
FC_REFLECT(cpsio::detail::faucet_testnet_create_account_params, (account)(keys));
FC_REFLECT(cpsio::detail::faucet_testnet_create_account_alternates_response, (alternates)(message));
FC_REFLECT(cpsio::detail::faucet_testnet_create_account_rate_limited_response, (message));
...
struct faucet_testnet_plugin_impl {
   ...
   results_pair find_alternates(const std::string& new_account_name) {
      ...
      const cpsio::detail::faucet_testnet_create_account_alternates_response response{
         names, "Account name is already in use."};
      return { conflict_with_alternates, fc::variant(response) };
   }

   results_pair create_account(const std::string& new_account_name, const cpsio::types::public_key& owner_pub_key, const cpsio::types::public_key& active_pub_key) {

      auto creating_account = database().find<account_object, by_name>(_create_account_name);
      CPS_ASSERT(creating_account != nullptr, transaction_exception,
      ...
      {
         cpsio::detail::faucet_testnet_create_account_rate_limited_response response{
      ...
      trx.scope = sort_names({_create_account_name,config::cps_contract_name});
      transaction_emplace_message(trx, config::cps_contract_name, std::vector<types::account_permission>{{_create_account_name,"active"}}, "newaccount",
      ...
      return std::make_pair(account_created, fc::variant(cpsio::detail::faucet_testnet_empty()));
   }

   results_pair create_faucet_account(const std::string& body) {
      const cpsio::detail::faucet_testnet_create_account_params params = fc::json::from_string(body).as<cpsio::detail::faucet_testnet_create_account_params>();
      return create_account(params.account, cpsio::types::public_key(params.keys.owner), cpsio::types::public_key(params.keys.active));
   }
...
const key_pair faucet_testnet_plugin_impl::_default_key_pair = {"CPS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"};
