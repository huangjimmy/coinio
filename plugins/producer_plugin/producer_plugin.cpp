...
public:
   ...
   uint32_t _production_skip_flags = cpsio::chain::chain_controller::skip_nothing;
   cpsio::chain::block_schedule::factory _production_scheduler = cpsio::chain::block_schedule::in_single_thread;
...
void new_chain_banner(const cpsio::chain::chain_controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   auto private_key_default = std::make_pair(chain::public_key_type(default_priv_key.get_public_key()),
                                             cpsio::utilities::key_to_wif(default_priv_key));
   ...
   command_line_options.add_options()
         ("scheduler", boost::program_options::value<string>()->default_value("single-thread")->notifier([this](const string& v){
             if (v == "single-thread") {
                my->_production_scheduler = cpsio::chain::block_schedule::in_single_thread;
             } else if (v == "cycling-conflicts") {
                ilog("Using scheduler by_cycling_conflicts");
                my->_production_scheduler = cpsio::chain::block_schedule::by_cycling_conflicts;
             } else if (v == "threading-conflicts") {
                ilog("Using scheduler by_threading_conflicts");
                my->_production_scheduler = cpsio::chain::block_schedule::by_threading_conflicts;
             } else {
...
{ try {
   ...
   if( options.count("private-key") )
   {
      ...
      for (const std::string& key_id_to_wif_pair_string : key_id_to_wif_pair_strings)
      {
         ...
         fc::optional<fc::ecc::private_key> private_key = cpsio::utilities::wif_to_key(key_id_to_wif_pair.second);
...
{ try {
   ...
   if (!my->_producers.empty())
   {
      ...
      if(my->_production_enabled)
      {
         ...
         my->_production_skip_flags |= cpsio::chain::chain_controller::skip_undo_history_check;
      }
...
block_production_condition::block_production_condition_enum producer_plugin_impl::maybe_produce_block(fc::mutable_variant_object& capture) {
   ...
   cpsio::types::account_name scheduled_producer = chain.get_scheduled_producer( slot );
   ...
   cpsio::chain::public_key_type scheduled_key = chain.get_producer(scheduled_producer).signing_key;
