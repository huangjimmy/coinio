...
public:
   ...
   static types::abi cps_abi; // cached for common use
   ...
   bool is_scope_relevant(const cpsio::types::vector<account_name>& scope);
...
types::abi db_plugin_impl::cps_abi;
...
namespace {
                ...
                const chain::message& msg)
  {
     ...
     try {
        ...
        if (msg.code == config::cps_contract_name) {
           abis.set_abi(db_plugin_impl::cps_abi);
        } else {
        ...
        try {
           ...
           elog("Unable to convert CPS JSON to MongoDB JSON: ${e}", ("e", e.what()));
           elog("  CPS JSON: ${j}", ("j", json));
        }
     } catch (fc::exception& e) {
...
void db_plugin_impl::_process_irreversible_block(const signed_block& block)
{
   ...
   if (processed == 0) {
      if (startup) {
         ...
         if (block_num == 1 && block.producer == config::cps_contract_name) {
            block_num = 0;
         }
      } else {
...
// For now providing some simple account processing to maintain cps_balance
void db_plugin_impl::update_account(const chain::message& msg) {
   ...
   if (msg.type == transfer) {
      ...
      asset from_balance = asset::from_string(from_account.view()["cps_balance"].get_utf8().value.to_string());
      asset to_balance = asset::from_string(to_account.view()["cps_balance"].get_utf8().value.to_string());
      from_balance -= cpsio::types::share_type(transfer.amount);
      to_balance += cpsio::types::share_type(transfer.amount);

      document update_from{};
      update_from << "$set" << open_document << "cps_balance" << from_balance.to_string()
                  << "updatedAt" << b_date{now}
                  << close_document;
      document update_to{};
      update_to << "$set" << open_document << "cps_balance" << to_balance.to_string()
                << "updatedAt" << b_date{now}
                << close_document;
      ...
      asset from_balance = asset::from_string(from_view["cps_balance"].get_utf8().value.to_string());
      from_balance -= newaccount.deposit;
      document update_from{};
      update_from << "$set" << open_document << "cps_balance" << from_balance.to_string() << close_document;
      ...
      asset balance = asset::from_string(from_account.view()["cps_balance"].get_utf8().value.to_string());
      asset unstack_balance = asset::from_string(from_account.view()["unstacking_balance"].get_utf8().value.to_string());
...
bool db_plugin_impl::is_scope_relevant(const cpsio::types::vector<account_name>& scope)
{
...
void db_plugin_impl::init() {
   ...
   cps_abi = native_contract::native_contract_chain_initializer::cps_contract_abi();
   ...
   if (accounts.count(doc.view()) == 0) {
      auto now = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::microseconds{fc::time_point::now().time_since_epoch().count()});
      doc << "name" << config::cps_contract_name.to_string()
          << "cps_balance" << asset(config::initial_token_supply).to_string()
          << "staked_balance" << asset().to_string()
          << "unstaking_balance" << asset().to_string()
          << "abi" << bsoncxx::from_json(fc::json::to_string(cps_abi))
          << "createdAt" << b_date{now}
          << "updatedAt" << b_date{now};

      if (!accounts.insert_one(doc.view())) {
         elog("Failed to insert account ${n}", ("n", config::cps_contract_name.to_string()));
      }
...
void db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   ...
   cfg.add_options()
         ...
         "The queue size between CPSd and MongoDB process thread.")
         ("mongodb-uri,m", bpo::value<std::string>(),
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database 'CPS' is used if not specified in URI.")
         ;
#endif
}
...
void db_plugin::plugin_initialize(const variables_map& options)
{
   ...
   if (options.count("mongodb-uri")) {
      ...
      wlog("cpsio::db_plugin configured, but no --mongodb-uri specified.");
