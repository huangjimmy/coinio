...
namespace native {
  /**
    @defgroup cpscontract CPS Contract
    @brief Documents the interface to the CPS currency contract
    @ingroup contracts
    @{
  */
   ...
   /**
    *   @class cps
    *   @brief A *native* currency contract implemented with account named `cps`
    *   @ingroup contracts
    *
    *   @details The CPS contract is a *native* currency contract implemented with account named `cps`. This contract enables
    *   users to transfer CPS tokens to each other. This contract is designed to work the @ref stakedcontract and
    *   @ref systemcontract when creating new accounts, claiming staked CPS.
    */
   struct cps : public contract {
      ...
      access to the `cps` scope which means that multiple transfers can execute in parallel
      ...
      other contracts are notified anytime CPS tokens are transferred.

      */
      void transfer (
         account_name from,  ///< account from which CPS will be withdrawn
         account_name to,    ///< account to receive CPS, may not be same as `from`
