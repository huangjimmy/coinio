...
namespace exchange {
   ...
   typedef cpsio::price<cps_tokens,currency_tokens>     price;

   //@abi table
   struct PACKED( bid ) {
      order_id           buyer;
      price              at_price;
      cpsio::tokens      quantity;
      time               expiration;

      void print() {
         cpsio::print( "{ quantity: ", quantity, ", price: ", at_price, " }" );
      }
   };
   ...
   struct PACKED( account ) {
      ...
      bool is_empty()const { return ! ( bool(cps_balance) | bool(currency_balance) | open_orders); }
   };

   using accounts = cpsio::table<N(exchange),N(exchange),N(account),account,uint64_t>;
