...
#include <cpslib/varint.hpp>
#include <cpslib/datastream.hpp>
...
namespace cpsio {
   ...
   */
   template<typename Stream> inline void pack( Stream& s, const bytes& value ) {
      cpsio::raw::pack( s, unsigned_int((uint32_t)value.len) );
      if( value.len )
   ...
   */
   template<typename Stream> inline void unpack( Stream& s, bytes& value ) {
      unsigned_int size; cpsio::raw::unpack( s, size );
      value.len = size.value;
      if( value.len ) {
         value.data = (uint8_t *)cpsio::malloc(value.len);
   ...
   */
   template<typename Stream> inline void pack( Stream& s, const string& v )  {
      ...
      cpsio::raw::pack( s, unsigned_int(size));
   ...
   */
   template<typename Stream> inline void pack( Stream& s, const price& v )  {
      cpsio::raw::pack(s, v.base);
      cpsio::raw::pack(s, v.quote);
   }
   ...
   */
   template<typename Stream> inline void unpack( Stream& s, price& v)  {
      cpsio::raw::unpack(s, v.base);
      cpsio::raw::unpack(s, v.quote);
   }
  /**
   ...
   */
   template<typename Stream> inline void pack( Stream& s, const asset& v )  {
      cpsio::raw::pack(s, v.amount);
      cpsio::raw::pack(s, v.symbol);
   }
   ...
   */
   template<typename Stream> inline void unpack( Stream& s, asset& v)  {
      cpsio::raw::unpack(s, v.amount);
      cpsio::raw::unpack(s, v.symbol);
   }
  /**
   ...
   *  @param v value to be serialized
   */
   template<typename Stream> inline void pack( Stream& s, const bool& v ) { cpsio::raw::pack( s, uint8_t(v) );             }
   ...
   */
   ...
   inline bytes pack(  const T& v ) {
      ...
      bytes b;
      b.len = ps.tellp();
      b.data = (uint8_t*)cpsio::malloc(b.len);
