...
namespace cpsio {
  ...
  *  @tparam currency - type of the currency (e.g. cps) represented as an unsigned 64 bit integer
  *  @ingroup tokens
  *
  ...
  *  Example:
  *  @code
  *  typedef cpsio::token<uint32_t, N(MyToken)> MyToken;
  *  MyToken  a(128);
  ...
  *  @{
  */
  template<typename NumberType, uint64_t currency = N(cps) >
  struct token {
    /**
    * Type of the currency (e.g. cps) represented as an unsigned 64 bit integer
    ...
    */
    ...
    /**
    * Print as string representation of the token (e.g. 1 CPS)
    * @brief Print as string
    */
    inline void print() {
      cpsio::print( quantity, " ", name(currency_type) );
    }
  };
  /// @}
  ...
  *  Example:
  *  @code
  *  typedef cpsio::token<uint64_t, N(MyBaseToken)> MyBaseToken;
  *  typedef cpsio::token<uint64_t, N(MyQuoteToken)> MyQuoteToken;
  ...
  */
  ...
  struct price
  {
    ...
    */
    friend QuoteToken operator / ( BaseToken b, const price& q ) {
      cpsio::print( "operator/ ", uint128(b.quantity), " * ", uint128( precision ), " / ", q.base_per_quote, "\n" );
    ...
    */
    friend BaseToken operator * ( const QuoteToken& b, const price& q ) {
      cpsio::print( "b: ", b, " \n" );
      cpsio::print( "operator* ", uint128(b.quantity), " * ", uint128( q.base_per_quote ), " / ", precision, "\n" );
    ...
    */
    inline void print() {
      cpsio::print( base_per_quote, ".", " ", name(base_token_type::currency_type), "/", name(quote_token_type::currency_type)  );
    }
  private:
    /**
  ...
  /**
  * @typedef tokens
  * @brief Defines cps tokens
  * @details Defines cps tokens
  */
  typedef cpsio::token<uint64_t,N(cps)>   tokens;

  /**
  *  @struct cpsio::transfer
  *  @brief The binary structure of the `transfer` message type for the `cps` contract.
  *  @ingroup tokens
  *
  *  @details
  *  Example:
