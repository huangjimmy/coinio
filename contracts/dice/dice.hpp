...
namespace dice {
   ...
   struct PACKED( account ) {
      ...
      account_name        owner;
      cps_tokens          cps_balance;
      uint32_t            open_offers = 0;

      bool is_empty()const { return ! ( bool(cps_balance) |  open_offers); }
   };

   using accounts = cpsio::table<N(dice),N(dice),N(account),account,uint64_t>;
   using global_dices = cpsio::table<N(dice),N(dice),N(global),global_dice,uint64_t>;
   using offers = cpsio::table<N(dice),N(dice),N(global),global_dice,offer_primary_key,uint128_t>;
