...
#include <cpslib/raw_fwd.hpp>

namespace cpsio { namespace raw {
...
namespace cpsio {
   ...
   Type current_message_ex() {
      ...
      char* data = (char *)cpsio::malloc(size);
      assert(data && read_message(data, size) == size, "error reading message");
      Type value;
      cpsio::raw::unpack(data, size, value);
      cpsio::free(data);
      return value;
   }
   ...
   void dump(const key_value2& value, int tab=0) {
      print_ident(tab);print("key:[");print("]\n");
      print_ident(tab);print("value:[");print("\n"); cpsio::dump(value.value, tab+1);print_ident(tab);print("]\n");
   }
   template<>
