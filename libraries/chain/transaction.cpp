...
cpsio::chain::transaction_id_type signed_transaction::id() const {
...
const signature_type& cpsio::chain::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id) {
   signatures.push_back(key.sign_compact(sig_digest(chain_id)));
   return signatures.back();
}

signature_type cpsio::chain::signed_transaction::sign(const private_key_type& key, const chain_id_type& chain_id)const {
...
cpsio::chain::digest_type signed_transaction::merkle_digest() const {
   digest_type::encoder enc;
   fc::raw::pack(enc, static_cast<const types::transaction&>(*this));
   return enc.result();
}
