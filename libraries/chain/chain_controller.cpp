...
#include <cps/chain/block_summary_object.hpp>
#include <cps/chain/global_property_object.hpp>
#include <cps/chain/key_value_object.hpp>
#include <cps/chain/action_objects.hpp>
#include <cps/chain/generated_transaction_object.hpp>
#include <cps/chain/transaction_object.hpp>
#include <cps/chain/producer_object.hpp>
#include <cps/chain/permission_link_object.hpp>
#include <cps/chain/authority_checker.hpp>
#include <cps/chain/rate_limiting_object.hpp>
...
#include <cps/utilities/rand.hpp>
...
{ try {
   ...
   CPS_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
void check_output(const T& expected, const T& actual, const path_cons_list& path) {
   try {
      CPS_ASSERT((expected == actual), block_tx_output_exception,
   ...
   for (const auto& message : trx.messages) {
      for (const auto& declaredAuthority : message.authorization) {
         if (!checker.satisfied(declaredAuthority)) {
            CPS_ASSERT(checker.satisfied(declaredAuthority), tx_missing_sigs,
...
#warning TODO: Use a real chain_id here (where is this stored? Do we still need it?)
   ...
   for (const auto& message : trx.messages)
      for (const auto& declaredAuthority : message.authorization) {
         ...
         if ((_skip_flags & skip_authority_check) == false) {
            ...
            CPS_ASSERT(getPermission(declaredAuthority).satisfies(minimumPermission, index), tx_irrelevant_auth,
   ...
   if (!allow_unused_signatures && (_skip_flags & skip_transaction_signatures) == false)
      CPS_ASSERT(checker.all_keys_used(), tx_irrelevant_sig,
...
void chain_controller::validate_scope( const transaction& trx )const {
   CPS_ASSERT(trx.scope.size() + trx.read_scope.size() > 0, transaction_exception, "No scope specified by transaction" );
   for( uint32_t i = 1; i < trx.scope.size(); ++i )
      CPS_ASSERT( trx.scope[i-1] < trx.scope[i], transaction_exception, "Scopes must be sorted and unique" );
   for( uint32_t i = 1; i < trx.read_scope.size(); ++i )
      CPS_ASSERT( trx.read_scope[i-1] < trx.read_scope[i], transaction_exception, "Scopes must be sorted and unique" );
...
void chain_controller::validate_uniqueness( const signed_transaction& trx )const {
   ...
   CPS_ASSERT(transaction == nullptr, tx_duplicate, "Transaction is not unique");
}
...
void chain_controller::validate_tapos(const transaction& trx)const {
   ...
   CPS_ASSERT(transaction_verify_reference_block(trx, tapos_block_summary.block_id), transaction_exception,
...
{ try {
   ...
   CPS_ASSERT(trx.expiration <= now + int32_t(chain_configuration.max_trx_lifetime),
   ...
   CPS_ASSERT(now <= trx.expiration, transaction_exception, "Transaction is expired",
...
#define RATE_LIMIT_ASSERT(tx_msgs_exceeded, type_str) \
      CPS_ASSERT(message_count <= rate_limit, tx_msgs_exceeded, \
...
void chain_controller::process_message(const transaction& trx, account_name code,
   ...
   const blockchain_configuration& chain_configuration = get_global_properties().configuration;
   if( is_producing() ) {
      CPS_ASSERT((fc::time_point::now() - start_time).count() < chain_configuration.max_trx_runtime, checktime_exceeded,
         "Transaction message exceeded maximum total transaction time of ${limit}ms", ("limit", chain_configuration.max_trx_runtime));
   }
   CPS_ASSERT(depth < chain_configuration.in_depth_limit, msg_resource_exhausted,
   ...
   if (parent_context == nullptr && (_skip_flags & skip_authority_check) == false)
      CPS_ASSERT(apply_ctx.all_authorizations_used(), tx_irrelevant_auth,
...
{ try {
   ...
   CPS_ASSERT(depth < chain_configuration.in_depth_limit, tx_resource_exhausted,
...
const producer_object& chain_controller::validate_block_header(uint32_t skip, const signed_block& next_block)const {
   CPS_ASSERT(head_block_id() == next_block.previous, block_validate_exception, "",
              ("head_block_id",head_block_id())("next.prev",next_block.previous));
   CPS_ASSERT(head_block_time() < next_block.timestamp, block_validate_exception, "",
   ...
   if (next_block.block_num() % config::blocks_per_round != 0) {
      CPS_ASSERT(next_block.producer_changes.empty(), block_validate_exception,
                 "Producer changes may only occur at the end of a round.");
   } else {
      using boost::is_sorted;
      using namespace boost::adaptors;
      CPS_ASSERT(is_sorted(keys(next_block.producer_changes)) && is_sorted(values(next_block.producer_changes)),
   ...
   if(!(skip&skip_producer_signature))
      CPS_ASSERT(next_block.validate_signee(producer.signing_key), block_validate_exception,
   ...
   if(!(skip&skip_producer_schedule_check)) {
      CPS_ASSERT(next_block.producer == producer.owner, block_validate_exception,
...
{ try {
   if (!_db.find<global_property_object>()) {
      _db.with_write_lock([this, &starter] {
         ...
         signed_block block{};
         block.producer = config::cps_contract_name;
...
producer_round chain_controller::calculate_next_round(const signed_block& next_block) {
   ...
   CPS_ASSERT(boost::range::equal(next_block.producer_changes, changes), block_validate_exception,
...
void chain_controller::update_last_irreversible_block()
{
   ...
   size_t offset = cps_percent(producer_objs.size(), config::percent100 - config::irreversible_threshold_percent);
