...
#include <cps/chain/permission_link_object.hpp>
 ...
 */
void apply_cps_newaccount(apply_context& context) {
   auto create = context.msg.as<types::newaccount>();
   context.require_authorization(create.creator);

   CPS_ASSERT( validate(create.owner), message_validate_exception, "Invalid owner authority");
   CPS_ASSERT( validate(create.active), message_validate_exception, "Invalid active authority");
   CPS_ASSERT( validate(create.recovery), message_validate_exception, "Invalid recovery authority");

   auto& db = context.mutable_db;

   auto existing_account = db.find<account_object, by_name>(create.name);
   CPS_ASSERT(existing_account == nullptr, account_name_exists_exception,
   ...
   CPS_ASSERT(creatorBalance.balance >= create.deposit.amount, message_validate_exception,
 ...
 *
 * @ingroup native_cps
 * @defgroup cps_cps_transfer cpsio::cps_transfer
 */
///@{


void apply_cps_transfer(apply_context& context) {
   auto transfer = context.msg.as<types::transfer>();

   try {
      CPS_ASSERT(transfer.amount > 0, message_validate_exception, "Must transfer a positive amount");
   ...
   try {
      ...
      CPS_ASSERT(from.balance >= transfer.amount, message_precondition_exception, "Insufficient Funds",
 ...
 */
void apply_cps_lock(apply_context& context) {
   auto lock = context.msg.as<types::lock>();

   CPS_ASSERT(lock.amount > 0, message_validate_exception, "Locked amount must be positive");

   context.require_scope(lock.to);
   context.require_scope(lock.from);
   context.require_scope(config::cps_contract_name);
   ...
   CPS_ASSERT( locker.balance >= lock.amount, message_precondition_exception,
              "Account ${a} lacks sufficient funds to lock ${amt} CPS", ("a", lock.from)("amt", lock.amount)("available",locker.balance) );
...
void apply_cps_unlock(apply_context& context) {
   auto unlock = context.msg.as<types::unlock>();

   context.require_authorization(unlock.account);

   CPS_ASSERT(unlock.amount >= 0, message_validate_exception, "Unlock amount cannot be negative");

   const auto& balance = context.db.get<staked_balance_object, by_owner_name>(unlock.account);

   CPS_ASSERT(balance.staked_balance  >= unlock.amount, message_precondition_exception,
...
void apply_cps_setcode(apply_context& context) {
   auto& db = context.mutable_db;
...
void apply_cps_claim(apply_context& context) {
   auto claim = context.msg.as<types::claim>();

   CPS_ASSERT(claim.amount > 0, message_validate_exception, "Claim amount must be positive");
   ...
   CPS_ASSERT(balance != nullptr, message_precondition_exception,
   ...
   CPS_ASSERT(now >= balanceReleaseTime, message_precondition_exception,
              "Cannot claim balance until ${releaseDate}", ("releaseDate", balanceReleaseTime));
   CPS_ASSERT(balance->unstaking_balance >= claim.amount, message_precondition_exception,
...
void apply_cps_setproducer(apply_context& context) {
   auto update = context.msg.as<types::setproducer>();
   context.require_authorization(update.name);
   CPS_ASSERT(update.name.good(), message_validate_exception, "Producer owner name cannot be empty");

   auto& db = context.mutable_db;
   auto producer = db.find<producer_object, by_owner>(update.name);

   if (producer) {
      CPS_ASSERT(producer->signing_key != update.key || producer->configuration != update.configuration,
...
void apply_cps_okproducer(apply_context& context) {
   FC_ASSERT(false, "voting for producers is disabled for this test network");
   auto approve = context.msg.as<types::okproducer>();
   CPS_ASSERT(approve.approve == 0 || approve.approve == 1, message_validate_exception,
   ...
   CPS_ASSERT(voter.producer_votes.contains<producer_slate>(), message_precondition_exception,
   ...
   CPS_ASSERT(slate.size < config::max_producer_votes, message_precondition_exception,
              "Cannot approve producer; approved producer count is already at maximum");
   if (approve.approve)
      CPS_ASSERT(!slate.contains(producer.ownerName), message_precondition_exception,
...
void apply_cps_setproxy(apply_context& context) {
...
void apply_cps_updateauth(apply_context& context) {
   auto update = context.msg.as<types::updateauth>();
   CPS_ASSERT(!update.permission.empty(), message_validate_exception, "Cannot create authority with empty name");
   CPS_ASSERT(update.permission != update.parent, message_validate_exception,
              "Cannot set an authority as its own parent");
   CPS_ASSERT(validate(update.new_authority), message_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.new_authority));
   if (update.permission == "active")
      CPS_ASSERT(update.parent == "owner", message_validate_exception, "Cannot change active authority's parent");
   if (update.permission == "owner")
      CPS_ASSERT(update.parent.empty(), message_validate_exception, "Cannot change owner authority's parent");
   ...
   if (permission) {
      CPS_ASSERT(parent_id == permission->parent, message_precondition_exception,
...
void apply_cps_deleteauth(apply_context& context) {
   auto remove = context.msg.as<types::deleteauth>();
   CPS_ASSERT(remove.permission != "active", message_validate_exception, "Cannot delete active authority");
   CPS_ASSERT(remove.permission != "owner", message_validate_exception, "Cannot delete owner authority");
   ...
   // CPS_ASSERT(declaredPermission.satisfies(permission, index), unsatisfied_permission,
   ...
   { // Check for children
      ...
      CPS_ASSERT(range.first == range.second, message_precondition_exception,
...
void apply_cps_linkauth(apply_context& context) {
   auto requirement = context.msg.as<types::linkauth>();
   CPS_ASSERT(!requirement.requirement.empty(), message_validate_exception, "Required permission cannot be empty");
   ...
   if (link) {
      CPS_ASSERT(link->required_permission != requirement.requirement, message_precondition_exception,
...
void apply_cps_unlinkauth(apply_context& context) {
   auto& db = context.mutable_db;
   ...
   auto link = db.find<permission_link_object, by_message_type>(linkKey);
   CPS_ASSERT(link != nullptr, message_precondition_exception, "Attempting to unlink authority, but no link found");
   if (context.controller.is_applying_block())
      db.remove(*link);
}
