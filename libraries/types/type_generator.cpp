...
using std::string;
namespace cpsio { using types::simple_symbol_table; }
...
string generate_wren( const cpsio::types::struct_t& s, cpsio::types::simple_symbol_table& symbols ) {
...
void generate_wren( cpsio::types::simple_symbol_table& ss, const char* outfile ) {
  //for( const auto& s : ss.order ) 
}

void generate_hpp( cpsio::types::simple_symbol_table& ss, const char* outfile ) {
   struct FakeField { std::string type; std::string name; };
   auto arrays_to_vectors = [](cpsio::types::field f) {
   ...
   out << "#pragma once\n";
   out << "namespace cpsio { namespace types {\n";
   for( const auto& s : ss.order ) {
   ...
   for( const auto& s : ss.order ) {
      ...
      const auto& st = ss.structs[s];
      if( st.base.size() ) {
        out << "FC_REFLECT_DERIVED( cpsio::types::" << s << ", (cpsio::types::" << st.base <<"), ";
      } else  {
        out << "FC_REFLECT( cpsio::types::" <<  std::setw(33) << s << ", ";
      }
      for( const auto& f : st.fields ) {
         out <<"("<<f.name<<")";
      }
...
int  count_fields( cpsio::types::simple_symbol_table& ss, const cpsio::types::struct_t& st ) {
   if( st.base.size() )
...
int main( int argc, char** argv ) {
  try {
     ...
     cpsio::types::simple_symbol_table ss;
     ss.parse(in);
