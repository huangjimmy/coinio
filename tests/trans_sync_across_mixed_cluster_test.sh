...
getHeadBlockNum()
{
  INFO="$(programs/cpsc/cpsc get info)"
...
# stand up cpsd cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --cpsd \"--plugin cpsio::wallet_api_plugin\" $launcherOpts
programs/launcher/launcher --cpsd "--plugin cpsio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/cpsc/cpsc --port $port2 get block 1)"
...
PASSWORD_INITA="$(programs/cpsc/cpsc wallet create --name inita)"
...
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/cpsc/cpsc wallet import --name inita $INITA_PRV_KEY
verifyErrorCode "cpsc wallet import"
programs/cpsc/cpsc wallet import --name inita $PRV_KEY1
verifyErrorCode "cpsc wallet import"
programs/cpsc/cpsc wallet import --name inita $PRV_KEY2
verifyErrorCode "cpsc wallet import"
programs/cpsc/cpsc wallet import --name inita $PRV_KEY3
verifyErrorCode "cpsc wallet import"
...
ACCOUNT_INFO="$(programs/cpsc/cpsc create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "cpsc create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/cpsc/cpsc get account testera)"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/cpsc/cpsc transfer inita testera 975321 "test transfer")"
	...
	TRANS_INFO="$(programs/cpsc/cpsc --port $port2 get transaction $TRANS_ID)"
	verifyErrorCode "cpsc get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
