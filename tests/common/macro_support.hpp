...
#define MKACCT_IMPL(chain, name, creator, active, owner, recovery, deposit) \
   { \
      cpsio::chain::signed_transaction trx; \
...
#define SETCODE3(chain, acct, wast) \
   { \
      auto wasm = cpsio::chain::wast_to_wasm(wast); \
      types::setcode handler; \
      handler.account = #acct; \
      handler.code.assign(wasm.begin(), wasm.end()); \
      cpsio::chain::signed_transaction trx; \
      trx.scope = sort_names({config::cps_contract_name, #acct}); \
      transaction_emplace_message(trx, config::cps_contract_name, vector<types::account_permission>{{#acct,"active"}}, \
                                  "setcode", handler); \
   }
...
#define STAKE4(chain, sender, recipient, amount) \
   { \
      cpsio::chain::signed_transaction trx; \
      trx.scope = sort_names( { #sender, #recipient, config::cps_contract_name } ); \
      transaction_emplace_message(trx, config::cps_contract_name, vector<types::account_permission>{{#sender, "active"}}, \
...
#define BEGIN_UNSTAKE3(chain, account, amount) \
   { \
      cpsio::chain::signed_transaction trx; \
      trx.scope = sort_names( { config::cps_contract_name } ); \
      transaction_emplace_message(trx, config::cps_contract_name, \
                         vector<types::account_permission>{{#account, "active"}}, \
...
#define MKPDCR4(chain, owner, key, cfg) \
   { \
      cpsio::chain::signed_transaction trx; \
      trx.scope = sort_names( {#owner, config::cps_contract_name} ); \
      transaction_emplace_message(trx, config::cps_contract_name, \
...
#define APPDCR4(chain, voter, producer, approved) \
   { \
      cpsio::chain::signed_transaction trx; \
      trx.scope = sort_names( {#voter, config::cps_contract_name} ); \
      transaction_emplace_message(trx, config::cps_contract_name,  \
