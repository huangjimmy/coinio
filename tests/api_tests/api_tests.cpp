...
void send_set_code_message(testing_blockchain& chain, types::setcode& handler, account_name account)
{
   ...
   trx.messages[0].code = config::cps_contract_name;
...
{ try {
      //auto wasm = assemble_wast( readFile2("/home/matu/Documents/Dev/cps/build/contracts/test_api/test_api.wast").c_str() );
...
BOOST_FIXTURE_TEST_CASE(test_page_memory_exceeded, testing_fixture)
{
   ...
   catch (cpsio::chain::page_memory_error& )
   {
      // expected behavior
   }
}
...
{ try { \
   ...
   Make_Blockchain(chain, 5000, \
         ::cpsio::chain_plugin::default_received_block_transaction_execution_time, \
         ::cpsio::chain_plugin::default_create_block_transaction_execution_time, chain_controller::txn_msg_limits {}); \
   chain.produce_blocks(2); \
   Make_Account(chain, testapi); \
...
{ try {
   ...
   transaction_emplace_message(trx, "cps",
   ...
   const auto& balance        = chain_db.get<cpsio::chain::balance_object,cpsio::chain::by_owner_name>( "acc1" );
   const auto& staked_balance = chain_db.get<cpsio::chain::staked_balance_object,cpsio::chain::by_owner_name>( "acc1" );

   // manually set balance so it can be verified in test_account
   chain_db.modify(balance, [](cpsio::chain::balance_object& bo) {
      bo.balance = 24;
   });
   chain_db.modify(staked_balance, [](cpsio::chain::staked_balance_object& sbo) {
