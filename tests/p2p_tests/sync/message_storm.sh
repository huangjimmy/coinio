...
if [ "$delay" == 0 ]; then
    ...
    programs/launcher/launcher -p $pnodes -n $total_nodes -s $topo -d $delay --cpsd "--log-level-net-plugin debug"
fi
...
while [ $port  -ne $endport ]; do
    programs/cpsc/cpsc --port $port get block 1 >> test.out 2>&1;
    port=`expr $port + 1`
done
...
sleep 18
programs/cpsc/cpsc --port 8888 get block 5
sleep 15
programs/cpsc/cpsc --port 8888 get block 10
sleep 15
programs/cpsc/cpsc --port 8888 get block 15
sleep 15
programs/cpsc/cpsc --port 8888 get block 20
sleep 15
