...
my $cps_home = defined $ENV{CPS_HOME} ? $ENV{CPS_HOME} : getcwd;
my $cpsd = $cps_home . "/programs/cpsd/cpsd";
my $cpsc = $cps_home . "/programs/cpsc/cpsc";

my $nodes = defined $ENV{CPS_TEST_RING} ? $ENV{CPS_TEST_RING} : "1";
my $pnodes = defined $ENV{CPS_TEST_PRODUCERS} ? $ENV{CPS_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$cps_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of cpsd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"CPS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = cpsio::producer_plugin\n";
        print $cfg "plugin = cpsio::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$cpsc transfer cps inita 10 >> cpsc.out 2>> cpsc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
