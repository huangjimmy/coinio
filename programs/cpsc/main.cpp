 ...
 *  @defgroup cpsclienttool CPS Command Line Client Reference
 *  @brief Tool for sending transactions and querying state from @ref cpsd
 *  @ingroup cpsclienttool
 */

/**
  @defgroup cpsclienttool

  @section intro Introduction to CPSC

  `cpsc` is a command line tool that interfaces with the REST api exposed by @ref cpsd. In order to use `cpsc` you will need to
  have a local copy of `cpsd` running and configured to load the 'cpsio::chain_api_plugin'.
...
Options:
  ...
  -H,--host TEXT=localhost    the host where cpsd is running
  -p,--port UINT=8888         the port where cpsd is running
  --wallet-host TEXT=localhost
                              the host where cps-walletd is running
  --wallet-port UINT=8888     the port where cps-walletd is running

Subcommands:
  create                      Create various items, on and off the blockchain
  get                         Retrieve various items and information from the blockchain
  set                         Set or update blockchain state
  transfer                    Transfer CPS from account to account
...
Usage: ./cpsc create account [OPTIONS] creator name OwnerKey ActiveKey
...
using namespace cpsio::utilities;
using namespace cpsio::client::help;
using namespace cpsio::client::localize;
using namespace cpsio::client::config;
using namespace boost::filesystem;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define CPSC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
types::message generate_nonce() {
   return message(N(cps),{}, N(nonce), types::nonce{generate_nonce_string()});
}
...
vector<types::account_permission> get_account_permissions(const vector<string>& permissions) {
   auto fixedPermissions = permissions | boost::adaptors::transformed([](const string& p) {
      ...
      CPSC_ASSERT(pieces.size() == 2, "Invalid permission: ${p}", ("p", p));
...
cpsio::chain_apis::read_only::get_info_results get_info() {
  return call(host, port, get_info_func ).as<cpsio::chain_apis::read_only::get_info_results>();
}
...
void create_account(name creator, name newaccount, public_key_type owner, public_key_type active, bool sign) {
      auto owner_auth   = cpsio::chain::authority{1, {{owner, 1}}, {}};
      auto active_auth  = cpsio::chain::authority{1, {{active, 1}}, {}};
      auto recovery_auth = cpsio::chain::authority{1, {}, {{{creator, "active"}, 1}}};

      uint64_t deposit = 1;

      signed_transaction trx;
      trx.scope = sort_names({creator,config::cps_contract_name});
      transaction_emplace_message(trx, config::cps_contract_name, vector<types::account_permission>{{creator,"active"}}, "newaccount",
...
types::message create_updateauth(const name& account, const name& permission, const name& parent, const authority& auth, const name& permissionAuth) {
   return message { config::cps_contract_name,
...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->set_callback([this] {
         ...
         if (is_delete) {
            send_transaction({create_deleteauth(account, permission, name(permissionAuth))}, {account, config::cps_contract_name}, skip_sign);
         } else {
            types::authority authority;
            if (boost::istarts_with(authorityJsonOrFile, "CPS")) {
            ...
            send_transaction({create_updateauth(account, permission, parent, authority, name(permissionAuth))}, {name(account), config::cps_contract_name}, skip_sign);
         }      
      });
   }
...
struct set_action_permission_subcommand {
   ...
   set_action_permission_subcommand(CLI::App* actionRoot) {
      ...
      permissions->set_callback([this] {
         ...
         if (is_delete) {
            send_transaction({create_unlinkauth(account, code, type)}, {account, config::cps_contract_name}, skip_sign);
         } else {
            name requirement = name(requirementStr);
            send_transaction({create_linkauth(account, code, type, requirement)}, {name(account), config::cps_contract_name}, skip_sign);
         }      
      });
   }
};
...
int main( int argc, char** argv ) {
   ...
   app.add_option( "--wallet-host", wallet_host, localized("the host where cps-walletd is running"), true );
   app.add_option( "--wallet-port", wallet_port, localized("the port where cps-walletd is running"), true );
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", cpsio::client::config::version_str)) << std::endl;
   });
   ...
   createProducer->set_callback([&account_name, &ownerKey, &permissions, &skip_sign] {
      ...
      signed_transaction trx;
      trx.scope = sort_names({config::cps_contract_name, account_name});
      transaction_emplace_message(trx, config::cps_contract_name, account_permissions,
   ...
   contractSubcommand->set_callback([&] {
      ...
      signed_transaction trx;
      trx.scope = sort_names({config::cps_contract_name, account});
      transaction_emplace_message(trx, config::cps_contract_name, vector<types::account_permission>{{account,"active"}},
                                           "setcode", handler);
   ...
   uint64_t amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer CPS from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending CPS"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving CPS"))->required();
   transfer->add_option("amount", amount, localized("The amount of CPS to send"))->required();
   ...
   transfer->set_callback([&] {
      ...
      transaction_emplace_message(trx, config::cps_contract_name,
   ...
   importWallet->set_callback([&wallet_name, &wallet_key] {
      ...
      CPSC_ASSERT( key, "invalid private key: ${k}", ("k",wallet_key) );
   ...
   benchmark_setup->set_callback([&]{
      ...
      CPSC_ASSERT( number_of_accounts > num_existing_accounts, fmter.str().c_str());
      ...
      if (num_existing_accounts == 0) {
      	CPSC_ASSERT( number_of_accounts >= 2, "must create at least 2 accounts" );
      }   
   ...
   try {
      ...
      if (errorString.find("Connection refused") != string::npos) {
         if (errorString.find(fc::json::to_string(port)) != string::npos) {
            std::cerr << localized("Failed to connect to cpsd at ${ip}:${port}; is cpsd running?", ("ip", host)("port", port)) << std::endl;
         } else if (errorString.find(fc::json::to_string(wallet_port)) != string::npos) {
            std::cerr << localized("Failed to connect to cps-walletd at ${ip}:${port}; is cps-walletd running?", ("ip", wallet_host)("port", wallet_port)) << std::endl;
         } else {
