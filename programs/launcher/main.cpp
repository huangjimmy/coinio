...
struct launcher_def {
  ...
  void assign_name (cpsd_def &node);
  ...
  void prep_remote_config_dir (cpsd_def &node, host_def *host);
  void launch (cpsd_def &node, string &gts);
...
launcher_def::set_options (bpo::options_description &cli) {
  cli.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "CPSD does not require transaction signatures.")
    ("cpsd", bpo::value<string>(&cpsd_extra_args), "forward cpsd command line argument(s) to each instance of cpsd, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many cpsd instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &cpsd : binding.instances) {
          cpsd.host = binding.host_name;
          cpsd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(cpsd.p2p_port);
          aliases.push_back (cpsd.name);
        }
      }
  ...
  char* erd_env_var = getenv ("CPS_ROOT_DIR");
...
void
launcher_def::assign_name (cpsd_def &node) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    ...
    for (size_t i = 0; i < total_nodes; i++) {
      ...
      assign_name(cpsd);
      aliases.push_back(cpsd.name);
      cpsd.set_host (&local_host);
      local_host.instances.emplace_back(move(cpsd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      if (ph_count == 0) {
        ...
        lhost->cps_root_dir =
          (local_id.contains (lhost->host_name) || servers.remote_cps_root.empty()) ?
          erd : servers.remote_cps_root;
      ...
      aliases.push_back(cpsd.name);
      cpsd.set_host (lhost);
      lhost->instances.emplace_back(move(cpsd));
      --ph_count;
    }
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  cpsd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bf::path dd = bf::path(host->cps_root_dir) / instance.data_dir;
    if (bf::exists (dd)) {
      if (force_overwrite) {
    ...
    bf::path dpath = bf::path (host->cps_root_dir) / instance.data_dir / "config.ini";
    ...
    dpath = bf::path (host->cps_root_dir) / instance.data_dir / "logging.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = cpsio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = cpsio::db_plugin\n";
  }
  cfg << "plugin = cpsio::chain_api_plugin\n"
      << "plugin = cpsio::account_history_plugin\n"
      << "plugin = cpsio::account_history_api_plugin\n";
  cfg.close();
}
...
void
launcher_def::prep_remote_config_dir (cpsd_def &node, host_def *host) {
  bf::path abs_data_dir = bf::path(host->cps_root_dir) / node.data_dir;
  string add = abs_data_dir.string();
  string cmd = "cd " + host->cps_root_dir;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->cps_root_dir
...
void
launcher_def::launch (cpsd_def &instance, string &gts) {
  ...
  bf::path pidf  = dd / "cpsd.pid";

  host_def* host = deploy_config_files (*instance.node);
  node_rt_info info;
  info.remote = !host->is_local();

  string cpsdcmd = "programs/cpsd/cpsd ";
  if (skip_transaction_signatures) {
    cpsdcmd += "--skip-transaction-signatures ";
  }
  if (!cpsd_extra_args.empty()) {
    cpsdcmd += cpsd_extra_args + " ";
  }

  if( add_enable_stale_production ) {
    cpsdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  cpsdcmd += "--data-dir " + instance.data_dir;
  if (gts.length()) {
    cpsdcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->cps_root_dir + "; nohup " + cpsdcmd + " > "
    ...
    string cmd = "cd " + host->cps_root_dir + "; kill -9 `cat " + pidf.string() + "`";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << cpsdcmd << endl;

    bp::child c(cpsdcmd, bp::std_out > reout, bp::std_err > reerr );
    bf::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << cpsdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(cpsd_def, string sig_opt) {
}
#endif
...
int main (int argc, char *argv[]) {
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << cpsio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(cps_root_dir)
...
FC_REFLECT( server_identities, (producer) (observer) (db) (remote_cps_root) (ssh) )
