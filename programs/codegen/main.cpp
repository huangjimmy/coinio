...
struct codegen {
   ...
   void generate_dump(ostringstream& output, const types::struct_t& type) {
      ...
      for(const auto& field : type.fields ) {
         ...
         if( abis.is_struct(field_type) ) {
            output << "print(\"\\n\"); cpsio::dump(value."<< field.name <<", tab+1);";
   ...
   void generate_current_message_ex(ostringstream& output) {
      ...
      output << tab << tab << "char* data = (char *)cpsio::malloc(size);" << endl;
      output << tab << tab << "assert(data && read_message(data, size) == size, \"error reading message\");" << endl;
      output << tab << tab << "Type value;" << endl;
      output << tab << tab << "cpsio::raw::unpack(data, size, value);" << endl;
      output << tab << tab << "cpsio::free(data);" << endl;
   ...
   void generate() {
      ...
      output << "#include <cpslib/types.hpp>" << endl;
      output << "#include <cpslib/message.hpp>" << endl;
      output << "#include <cpslib/datastream.hpp>" << endl;
      output << "#include <cpslib/raw_fwd.hpp>" << endl;
      
      output << endl;

      //Generate serialization/deserialization for every cotract type
      output << "namespace cpsio { namespace raw {" << endl;
      ...
      output << "#include <cpslib/raw.hpp>" << endl;

      output << "namespace cpsio {" << endl;
